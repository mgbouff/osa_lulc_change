...
change_87_98_tidy <- lu_change_87_98 %>% 
  ...
  select(source, target, change_87_98)
...
change_98_19_tidy <- lu_change_98_19 %>% 
  ...
  rename("source" = land_use) %>% 
  select(source, target, change_98_19)
...
# full_sankey <- change_full %>% 
#   pivot_longer(!source:target, names_to = "step", values_to = "change")

sankey_full <- merge(change_87_98_name, change_98_19_name, by = c("source", "target"))

# Links
links_combined <- data.frame(
  source= full_sankey$source,
  target= full_sankey$target,
  value= full_sankey$change
  )
